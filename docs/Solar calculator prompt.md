...
1. Use the Single Battery DC Volts to calculate series/parallel battery strings and total Bank Ah.
...
6. Calculate Charging Time Metrics: Available solar amps minus active load to find net charging amps. Use 50% of Bank Ah as the capacity to replenish to estimate full charge time in hours.

Display the results in a card showing the system goal, target current, exact/rounded panels needed, recommended array wiring (e.g., 2S2P), MPPT ratings, charging time metrics (bank capacity, replenish capacity, net amps, charge time), and a clear mechanical engineering explanation of the wiring choice.